...
#include "periodicTranslationConstraint.H"
...
namespace Foam
{
defineTypeNameAndDebug(periodicTranslationConstraint, 0);
...
periodicTranslationConstraint::periodicTranslationConstraint(
...
{
    cType_   = acceleration;
    nameStr_ = "periodicTranslationConstraint class";
...
void periodicTranslationConstraint::constrain(volumetricParticle& particle) const
{
	scalar t = obr_.time().value();
	int sig = sign(sin(M_PI * t / period_)) ;

  particle.getAcceleration() =
         ( sig * staticVelocity_ - particle.getVelocity() ) / obr_.time().deltaT().value();

  _PDBO_("time = " << t << "\tsign = " << sig << "\t")
}
