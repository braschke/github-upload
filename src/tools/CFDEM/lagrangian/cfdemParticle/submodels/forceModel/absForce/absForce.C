...
defineTypeNameAndDebug(absForce, 0);
...
:
    ...
    voidFracFieldName_(propsDict_.lookup("voidFracFieldName")),
    voidFrac_(sm.mesh().lookupObject<volScalarField> (voidFracFieldName_)),
    addedMassCoeff_(0.0)
{
    // block absForceModel for type B
    if (modelType_ == "B") FatalError <<"using  model absForce with model type B is not valid\n" << abort(FatalError);
    ...
    if (propsDict_.found("useAddedMass")) 
    {
        ...
        Info << "absForce will also include added mass with coefficient: " << addedMassCoeff_ << endl;
    ...
    if(probeIt_)
    {
        ...
        particleCloud_.probeM().vectorFields_.append("absForce"); //first entry must the be the force
...
void absForce::setForce() const
{
    //const volVectorField& gradVoidFrac_ = forceSubM(0).gradVoidFracField(voidFrac_);
    volVectorField gradVoidFrac_ = -fvc::grad(voidFrac_);
    ...
    for(int index = 0;index <  particleCloud_.numberOfParticles(); index++)
    {
        ...
        //{
            ...
            if (cellI > -1) // particle Found
            {
                ...
                {
                    gradVoidFrac = gradVoidFracInterpolator_().interpolate(position,cellI);
                }else
                {
                    gradVoidFrac = gradVoidFrac_[cellI];
                }*/
                ...
                force = -Vs*gradVoidFrac*(1.0+addedMassCoeff_);

                if(forceSubM(0).verbose() && index >0 && index <2)
                {
                    Info << "index = " << index << endl;
                    Info << "gradVoidFrac = " << gradVoidFrac << endl;
