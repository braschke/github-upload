...
#include "LSMdynamicRefineFvMesh.H"
...
namespace Foam
{
    defineTypeNameAndDebug(LSMdynamicRefineFvMesh, 0);
    addToRunTimeSelectionTable(dynamicFvMesh, LSMdynamicRefineFvMesh, IOobject);
}
...
Foam::label Foam::LSMdynamicRefineFvMesh::count
(
...
void Foam::LSMdynamicRefineFvMesh::calculateProtectedCells
(
...
void Foam::LSMdynamicRefineFvMesh::readDict()
{
    ...
    maxTimeAutoRefine_ = refineDict.lookupOrDefault<label>
                                                      ("maxTimeAutoRefine", -1);
}
...
Foam::LSMdynamicRefineFvMesh::refine
(
...
)
{
    ...
    if (debug)
    {
        ...
        for (label faceI = 0; faceI < nInternalFaces(); faceI++)
        {
            ...
            {
                FatalErrorIn("LSMdynamicRefineFvMesh::refine(const labelList&)")
    ...
    */
    ...
    // have been renumbered will already have been handled by the mapping.
    {
        ...
        forAll(faceMap, faceI)
        {
            ...
            if (oldFaceI >= 0)
            {
                ...
                if (masterFaceI < 0)
                {
                    FatalErrorIn
                    (
                        "LSMdynamicRefineFvMesh::refine(const labelList&)"
        ...
        forAllIter(HashTable<surfaceScalarField*>, fluxes, iter)
        {
            ...
            {
                WarningIn("LSMdynamicRefineFvMesh::refine(const labelList&)")
...
//                sigFpe::fillNan(phi.internalField()); //Original (OF-3)
//Vora:
...
// internalField and internalFieldRef have been replaced by direct calls to the field variable. Eg: U.internalField() becomes U() for a const reference to the internal field, or U.ref() for non-const access.
                ...
                sigFpe::fillNan(phi.ref()); //New OF-5
...
//OF-3            surfaceScalarField::GeometricBoundaryField& bphi =
//                phi.boundaryField();

//Vora
//In OF-5.x surfaceScalarField::GeometricBoundaryField is just surfaceScalarField::Boundary.
//In OF-5.x boundaryField() returns const-reference to the boundary field; and boundaryFieldRef() returns a reference to the boundary field.

            surfaceScalarField::Boundary& bphi =
                phi.boundaryFieldRef(); // non-const reference to the boundary field.
...
// (an approximate) flux
Foam::autoPtr<Foam::mapPolyMesh>
Foam::LSMdynamicRefineFvMesh::unrefine
(
...
Foam::scalarField
Foam::LSMdynamicRefineFvMesh::maxPointField(const scalarField& pFld) const
{
...
Foam::scalarField
Foam::LSMdynamicRefineFvMesh::maxCellField(const List<scalarField*> &vFlds) const
{
    scalarField pFld(nPoints(), -GREAT);

    forAll(vFlds, fieldI)
    {
      const scalarField &vFld = *vFlds[fieldI];
...
Foam::scalarField
Foam::LSMdynamicRefineFvMesh::cellToPoint(const scalarField& vFld) const
{
...
Foam::scalarField Foam::LSMdynamicRefineFvMesh::error
(
...
void Foam::LSMdynamicRefineFvMesh::selectRefineCandidates
(
...
) const
{
    ...
    {
        extendMarkedCells(candidateCell);
    }

    // autorefinement: a region can be defined to be automatically refined
    // You may specify a limiting timestep until which autorefinement will happen
    // if maxTimeAutoRefine_ == -1 then do autorefinement in any timestep
    if( maxTimeAutoRefine_ >= 0 && time().timeIndex() > maxTimeAutoRefine_ )
      return;
    // Check if cellZone 'alwaysRefine' is available
    // If 'yes' mark cells belonging to zone as refinement candidates
    const cellZoneMesh &  cZMesh = cellZones();
    label zoneID = cZMesh.findZoneID("alwaysRefine");
    if( zoneID == -1 ) // zone not existent
      return;

    // get cellZone
    const cellZone& cZ = cZMesh[zoneID];
    // Loop over all cells in zone and mark as candidate
    forAll(cZ, cellI)
    {
        candidateCell.set(cZ[cellI], 1);
    }
}


Foam::labelList Foam::LSMdynamicRefineFvMesh::selectRefineCells
(
...
Foam::labelList Foam::LSMdynamicRefineFvMesh::selectUnrefinePoints
(
...
void Foam::LSMdynamicRefineFvMesh::extendMarkedCells
(
...
void Foam::LSMdynamicRefineFvMesh::checkEightAnchorPoints
(
...
Foam::LSMdynamicRefineFvMesh::LSMdynamicRefineFvMesh(const IOobject& io)
:
    dynamicFvMesh(io),
    meshCutter_(*this),
    dumpLevel_(false),
...
Foam::LSMdynamicRefineFvMesh::~LSMdynamicRefineFvMesh()
{}


// * * * * * * * * * * * * * * * Member Functions  * * * * * * * * * * * * * //

bool Foam::LSMdynamicRefineFvMesh::update()
{
    ...
    {
        FatalErrorIn("LSMdynamicRefineFvMesh::update()")
    ...
    if (time().timeIndex() > 0 && time().timeIndex() % refineInterval == 0)
    {
        ...
        const wordList fieldNames(refineDict.lookup("fields"));

        List<scalarField*> vFlds(fieldNames.size());

        forAll(vFlds, fieldI)
        {
          vFlds[fieldI] = const_cast<volScalarField*>(
                               &lookupObject<volScalarField>(fieldNames[fieldI])
                                                  );
        }
...
//Vora OF-3 Version
//bool Foam::LSMdynamicRefineFvMesh::writeObject
//(
...
//Vora OF-5.x: new structure
    bool writeOk =
    (
        dynamicFvMesh::writeObject(fmt, ver, cmp, valid) //fourth entity "(const bool) valid"
     && meshCutter_.write()
    );
